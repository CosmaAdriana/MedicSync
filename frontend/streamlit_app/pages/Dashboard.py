...
try:
    ...
    # Tabel departamente
    st.markdown("##### 🏛️ Situație pe Departamente")
    if hospital_stats:
    ...
    if user_role in ("nurse", "doctor") and user_dept_id:
        ...
        st.subheader(f"🏷️ Secția Mea — {my_dept_name}")
        ...
        with mc1: kpi_card("Internați în secție",  str(len(my_admitted)),   "", "green")
        with mc2: kpi_card("Critici în secție",     str(len(my_critical)),   "", "red")
        with mc3: kpi_card("Externați în secție",   str(len(my_discharged)), "", "blue")

        if my_patients:
            st.markdown("##### Pacienți activi în secția ta")
